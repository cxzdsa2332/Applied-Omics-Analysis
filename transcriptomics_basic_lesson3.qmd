...
# featureCounts output: first 6 columns are metadata (GeneID, Chr, Start, End, Strand, Length)
# Columns 7+ are per-sample counts, named by full BAM path
BASE <- "/Users/angdong/Documents/bio_example/bulkRNA"
count_matrix <- read.table(
  file.path(BASE, "results/featureCounts/pasilla_counts.txt"),
  header = TRUE, skip = 1, row.names = 1
)
counts <- count_matrix[, 7:ncol(count_matrix)]

# Strip full paths; keep only SRR accession as column name
# e.g. ".../results/star/SRR1039508/Aligned.sortedByCoord.out.bam" → "SRR1039508"
colnames(counts) <- sub(".*/(SRR[0-9]+)/.*", "\\1", colnames(counts))

# Sample metadata: airway dataset (Himes et al. 2014)
# SRR1039508, SRR1039512 = untreated control
# SRR1039516              = dexamethasone treated
# Note: 3 samples is minimally powered; full analysis uses all 6 PE samples
sample_info <- data.frame(
  condition = factor(
    c("untreated", "untreated", "treated"),
    levels = c("untreated", "treated")
  ),
  row.names = c("SRR1039508", "SRR1039512", "SRR1039516")
)
...
# Reference level already set via factor(levels = ...) above; relevel is redundant but harmless
