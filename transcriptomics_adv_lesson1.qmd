...
1. **读段计数**：对每个可变剪接事件，统计每个样本的 inclusion junction reads（$I_k$）和 skipping junction reads（$S_k$）。
2. **Beta 二项分布建模**：对每个组，假设每个样本的读段计数服从 Beta-Binomial 分布，其中 Beta 先验参数刻画了组内的变异性。
3. **$\Delta\Psi$ 的后验估计**：通过贝叶斯推断得到 $\Delta\Psi$ 的后验分布，报告后验均值与置信区间。
4. **显著性检验**：计算 $|\Delta\Psi| > \delta$（通常 $\delta = 0.05$）的后验概率；同时报告 FDR 校正后的 P 值。
...
rMATS 采用两步运行策略：第一步仅计数（`--statoff`），第二步在计数基础上执行统计检验。两步分离的设计允许在多个比较组之间复用同一批比对结果，节省计算时间。
...
# Step 1: counting only (can be parallelized across experiments)
...
# Preview SE results
head -2 ${OUTPUT_DIR}/SE.MATS.JC.txt | cut -f1-12
```
...
# Read SE results
se <- read.table("rmats_output/SE.MATS.JC.txt",
...
#| fig-cap: "rMATS 可变外显子跳跃事件的火山图。横轴为 ΔPSI，纵轴为 -log₁₀(FDR)，红色点为显著差异事件（|ΔPSI| > 0.1，FDR < 0.05）。"
library(dplyr)
library(ggplot2)

# Simulate rMATS-like SE results for rendering demonstration
set.seed(42)
n_events <- 3000
se <- data.frame(
    geneSymbol          = paste0("Gene", seq_len(n_events)),
    IncLevelDifference  = rnorm(n_events, 0, 0.15),
    FDR                 = runif(n_events, 1e-6, 1)
)
se$FDR[abs(se$IncLevelDifference) > 0.2] <-
    runif(sum(abs(se$IncLevelDifference) > 0.2), 1e-6, 0.05)
...
```bash
# Generate Sashimi plot for top significant SE event
# Requires rmats2sashimiplot (conda install -c bioconda rmats2sashimiplot)
# Coordinates are taken from the top row of sig_se
TOP_GENE="GENE_SYMBOL"  # replace with actual top gene
COORDS="chr:start:end:strand"  # replace with actual coordinates

rmats2sashimiplot \
    --b1 "$(cat group1_bams.txt)" \
    --b2 "$(cat group2_bams.txt)" \
    --event-type SE \
    --event-coordinates ${COORDS} \
    --l1 "Group1" \
    --l2 "Group2" \
    --min-counts 5 \
    --output sashimi_output/
```
