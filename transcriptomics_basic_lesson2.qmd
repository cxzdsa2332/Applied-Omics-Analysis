...
# Requires ~30 GB RAM for human genome (GRCh38)
# --sjdbOverhang: read length - 1 (for 150 bp reads: 149)
mkdir -p index/STAR_GRCh38

STAR \
  --runMode genomeGenerate \
  --genomeDir index/STAR_GRCh38 \
  --genomeFastaFiles Homo_sapiens.GRCh38.dna.primary_assembly.fa \
  --sjdbGtfFile Homo_sapiens.GRCh38.109.gtf \
  --sjdbOverhang 149 \
  --runThreadN 16

# === Step 2: Align paired-end reads (with 2-pass mode) ===
mkdir -p results/star/SRR1039508
...
STAR \
  ...
  --outFileNamePrefix results/star/SRR1039508/ \
...
samtools index results/star/SRR1039508/Aligned.sortedByCoord.out.bam
...
```bash
# === Complete pipeline for all 6 paired-end pasilla samples ===
# Assumes STAR index is already built

SAMPLES=("SRR1039508" "SRR1039512" "SRR1039516"
         "SRR1039509" "SRR1039513" "SRR1039517")
...
for SAMPLE in "${SAMPLES[@]}"; do
  ...
  salmon quant \
    --index index/salmon_GRCh38 \
    --libType A \
    --mates1 data/trimmed/${SAMPLE}_1.fastq.gz \
    --mates2 data/trimmed/${SAMPLE}_2.fastq.gz \
    --output results/salmon/${SAMPLE} \
    --gcBias --seqBias --validateMappings \
    --threads 16
done

# === featureCounts on all BAM files simultaneously ===
featureCounts \
  -T 16 \
  -a Homo_sapiens.GRCh38.109.gtf \
  -o results/featureCounts/pasilla_counts.txt \
...
当一条 read 能比对到基因组多个位置时，称为**多比对 read（multimapper）**。多比对的主要来源：
...
| 工具 | 策略 | 适用场景 |
|---|---|---|
| STAR（默认） | 保留所有位置，`NH:i:N` 标记总数 | 保留信息，下游工具决策 |
| featureCounts | 丢弃（`-M` 保留） | 保守计数 |
| featureCounts + `--fraction` | 按位置数均等分配（产生小数） | 估计总体趋势，不适合 DEA |
| Salmon | EM 算法优化分配 | 转录本级别定量首选 |

```bash
# Check multimapping rate from STAR log
grep "% of reads mapped to multiple loci" \
  results/star/SRR1039508/Log.final.out

# Count uniquely vs multi-mapped reads
samtools view -c -q 255 sample.bam   # uniquely mapped (MAPQ=255 in STAR)
samtools view -c -F 4 sample.bam     # all mapped
```
